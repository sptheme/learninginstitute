...
if ( ! class_exists( 'WPSP_Cp_Publications' ) ) {
	class WPSP_Cp_Publications {
		...
		public function __construct() {
			...
			// Update vars
			$this->label = get_option( 'publications_labels' );
			$this->label = $this->label ? $this->label : _x( 'Publications', 'Publications Post Type Label', 'wpsp_admin' );

			// Adds the publications post type
			add_action( 'init', array( $this, 'register_post_type' ), 0 );

			// Adds the publications taxonomies
			if ( 'off' != get_option( 'publications_tags', 'on' ) ) {
				add_action( 'init', array( $this, 'register_tags' ), 0 );
			}
			if ( 'off' != get_option( 'publications_categories', 'on' ) ) {	
			...
			if ( is_admin() ) {
				...
				add_filter( 'manage_edit-publications_columns', array( $this, 'edit_columns' ) );
				add_action( 'manage_publications_posts_custom_column', array( $this, 'column_display' ), 10, 2 );
				...
				add_action( 'admin_print_styles-publications_page_wpsp-publications-editor', array( $this,'css' ) );
			}	

			// Adds the publications custom sidebar
		...
		public function register_post_type() {
			...
			$name             = $this->label;
			$singular_name    = get_option( 'publications_singular_name' );
			$singular_name    = $singular_name ? $singular_name : esc_html__( 'Publication Item', 'wpsp_admin' );
			$slug  			  = get_option( 'publications_slug' );
			...
			$args = array(
				...
				'menu_position' => 20,
				'exclude_from_search' => $publications_search,
				'rewrite' => array(
					'slug' => $slug,
				),
				'supports' => array(
			...
			register_post_type( 'publications', apply_filters( 'wpsp_publications_args', $args ) );

		}

		/**
		 * Register Publications tags.
		 *
		 * @since 1.0.0
		 */
		public static function register_tags() {
			...
			$name = esc_html( get_option( 'publications_tag_labels' ) );
			$name = $name ? $name : esc_html__( 'Publications Tags', 'wpsp' );
			$slug = get_option( 'publications_tag_slug' );
			$slug = $slug ? $slug : 'publications-tag';

			// Define publications tag arguments
			$args = array(
				'labels' => array(
					'name' => $name,
					'singular_name' => $name,
			...
			// Apply filters
			$args = apply_filters( 'wpsp_taxonomy_publications_tag_args', $args );

			// Register the publications tag taxonomy
			register_taxonomy( 'publications_tag', array( 'publications' ), $args );

		}

		/**
		 * Register Publications category.
		 *
		 * @since 1.0.0
		 */
		public static function register_categories() {
			...
			// Apply filters
			$args = apply_filters( 'wpsp_taxonomy_publications_category_args', $args );

			// Register the publications category taxonomy
		...
		public static function edit_columns( $columns ) {
			if ( taxonomy_exists( 'publications_category' ) ) {
				$columns['publications_category'] = esc_html__( 'Category', 'wpsp' );
			}
			if ( taxonomy_exists( 'publications_tag' ) ) {
				$columns['publications_tag']      = esc_html__( 'Tags', 'wpsp' );
			}
			return $columns;
		}
		...
		public static function column_display( $column, $post_id ) {
			...
			switch ( $column ) :
				...
				// Display the publications categories in the column view
				case 'publications_category':

					if ( $category_list = get_the_term_list( $post_id, 'publications_category', '', ', ', '' ) ) {
						echo $category_list;
					} else {
						echo '&mdash;';
					}
				...
				// Display the publications tags in the column view
				case 'publications_tag':

					if ( $tag_list = get_the_term_list( $post_id, 'publications_tag', '', ', ', '' ) ) {
						echo $tag_list;
					} else {
						echo '&mdash;';
					}
		...
		/**
		 * Adds taxonomy filters to the publications admin page.
		 *
		 * @since 1.0.0
		 */
		public static function tax_filters() {
			...
			$taxonomies = array( 'publications_category', 'publications_tag' );

			// must set this to the post type you want the filter(s) displayed on
			if ( 'publications' == $typenow ) {
		...
		/**
		 * Add sub menu page for the Publications Editor.
		 *
		 * @since 1.0.0
		 * @link    http://codex.wordpress.org/Function_Reference/add_theme_page
		 */
		public function add_page() {
			$wpsp_publications_editor = add_submenu_page(
				'edit.php?post_type=publications',
				esc_html__( 'Post Type Editor', 'wpsp' ),
				esc_html__( 'Post Type Editor', 'wpsp' ),
				'administrator',
				'wpsp-publications-editor',
				array( $this, 'create_admin_page' )
			);
			add_action( 'load-'. $wpsp_publications_editor, array( $this, 'flush_rewrite_rules' ) );
		}
		...
		public static function flush_rewrite_rules() {
			...
			if ( $screen->id == 'publications_page_wpsp-publications-editor' ) {
				flush_rewrite_rules();
			}
		...
		/**
		 * Function that will register the publications editor admin page.
		 *
		 * @since 1.0.0
		 * @link    http://codex.wordpress.org/Function_Reference/register_setting
		 */
		public function register_page_options() {
			register_setting( 'wpsp_publications_options', 'wpsp_publications_branding', array( $this, 'sanitize' ) );
		}
		...
		public static function notices() {
			settings_errors( 'wpsp_publications_editor_page_notices' );
		}
		...
		public static function sanitize( $options ) {
			...
			// Add notice
			add_settings_error(
				'wpsp_publications_editor_page_notices',
		...
		/**
		 * Output for the actual Publications Editor admin page.
		 *
		 * @since 1.0.0
		 */
		public static function create_admin_page() { ?>
			<div class="wrap">
				...
				<?php settings_fields( 'wpsp_publications_options' ); ?>
				<table class="form-table">
					<tr valign="top">
						...
						<td>
							...
							<input type="hidden" name="wpsp_publications_branding[publications_admin_icon]" id="wpsp-dashicon-select-input" value="<?php echo esc_attr( $mod ); ?>"></td>
						</td>
					</tr>
					<tr valign="top">
						<th scope="row"><?php esc_html_e( 'Post Type: Name', 'wpsp' ); ?></th>
						<td><input type="text" name="wpsp_publications_branding[publications_labels]" value="<?php echo get_option( 'publications_labels' ); ?>" /></td>
					</tr>
					<tr valign="top">
						<th scope="row"><?php esc_html_e( 'Post Type: Singular Name', 'wpsp' ); ?></th>
						<td><input type="text" name="wpsp_publications_branding[publications_singular_name]" value="<?php echo get_option( 'publications_singular_name' ); ?>" /></td>
					</tr>
					<tr valign="top" id="wpsp-tags-enable">
						<th scope="row"><?php esc_html_e( 'Enable Tags', 'wpsp' ); ?></th>
						<?php
						// Get checkbox value
						$mod = get_option( 'publications_tags', 'on' );
						$mod = 'off' != $mod ? 'on' : 'off'; // sanitize ?>
						<td><input type="checkbox" name="wpsp_publications_branding[publications_tags]" value="<?php echo esc_attr( $mod ); ?>" <?php checked( $mod, 'on' ); ?> /></td>
					</tr>
					<tr valign="top" id="wpsp-tags-label">
						<th scope="row"><?php esc_html_e( 'Tags: Label', 'wpsp' ); ?></th>
						<td><input type="text" name="wpsp_publications_branding[publications_tag_labels]" value="<?php echo get_option( 'publications_tag_labels' ); ?>" /></td>
					</tr>
					<tr valign="top" id="wpsp-tags-slug">
						<th scope="row"><?php esc_html_e( 'Tags: Slug', 'wpsp' ); ?></th>
						<td><input type="text" name="wpsp_publications_branding[publications_tag_slug]" value="<?php echo get_option( 'publications_tag_slug' ); ?>" /></td>
					</tr>
					<tr valign="top" id="wpsp-categories-enable">
						<th scope="row"><?php esc_html_e( 'Enable Categories', 'wpsp' ); ?></th>
						<?php
						// Get checkbox value
						$mod = get_option( 'publications_categories', 'on' );
						$mod = 'off' != $mod ? 'on' : 'off'; // sanitize ?>
						<td><input type="checkbox" name="wpsp_publications_branding[publications_categories]" value="<?php echo esc_attr( $mod ); ?>" <?php checked( $mod, 'on' ); ?> /></td>
					</tr>
					<tr valign="top" id="wpsp-categories-label">
						<th scope="row"><?php esc_html_e( 'Categories: Label', 'wpsp' ); ?></th>
						<td><input type="text" name="wpsp_publications_branding[publications_cat_labels]" value="<?php echo get_option( 'publications_cat_labels' ); ?>" /></td>
					</tr>
					<tr valign="top" id="wpsp-categories-slug">
						<th scope="row"><?php esc_html_e( 'Categories: Slug', 'wpsp' ); ?></th>
						<td><input type="text" name="wpsp_publications_branding[publications_cat_slug]" value="<?php echo get_option( 'publications_cat_slug' ); ?>" /></td>
					</tr>
				</table>
				<?php submit_button(); ?>
				</form>	
		...
		/**
		 * Registers a new custom publications sidebar.
		 *
		 * @since 1.0.0
		 */
		public static function register_sidebar() {
			...
			$obj            = get_post_type_object( 'publications' );
...
$wpsp_cp_publications = new WPSP_Cp_Publications;